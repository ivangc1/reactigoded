...
**Cerrado (SAME-FILE) + residual honesto (CROSS-MODULE) — `import X = NS.Y` que aliasa un TIPO** (`import window = Cfg.window` con `Cfg.window` type → sombra fantasma del global; el binding se borra al emit, `window.*` resuelve al global real). La versión PREVIA de este ADR metía TODO `import X = NS.Y` en el cubo "indecidible" — **eso era cobardía en la mitad decidible**, y un re-hunt (B4) lo cazó: confesé "decidible con el checker, elegimos no resolverlo" pero ni noté que para same-file no es ni un choice de checker, es **gratis** con los helpers que ya existen. La CLASE se parte limpia por la regla que el shadow-set ya enuncia ("**excluye lo PROVABLEMENTE borrado, decidible sintácticamente**"):

- **SAME-FILE** (`namespace Cfg { export interface window {} } import window = Cfg.window`) es **provablemente borrado sin checker**: el namespace `Cfg` está EN EL ARCHIVO, su miembro `window` es visiblemente una `interface`/`type`, decidible recorriendo el AST con los helpers que YA existen (`producesRuntimeValue` + `namespaceIsInstantiated`). Resolver un namespace del mismo archivo ES trabajo sintáctico de-un-solo-archivo. Pertenece a la misma clase erased-shadow que type-only import / `declare` / namespace type-only, que ya cerramos. → **CERRADO**: `importEqualsProducesRuntimeValue` resuelve el RHS contra los namespaces del archivo y reusa `producesRuntimeValue`; miembro-tipo no precarga (flaggea), miembro-valor precarga (clean), nested `A.B.C` igual.
- **CROSS-MODULE** (`import * as Ext from "./other"; import window = Ext.window`) SÍ exige el type-checker: el target vive en otro módulo, no resoluble con `createSourceFile`. Aquí el discriminador honesto es **"decidible CON el checker, elegimos gate sintáctico-puro"** — la autocrítica del round anterior, aplicada donde de verdad aplica. → **residual**, conservador value-alias (no flaggea — excluirlo FP-earía todo value-alias legítimo cross-module). La pureza sintáctica es la línea que separa same-file (in-bounds → cerrar) de cross-module (out-of-bounds → residual).

El namespace (decidible → excluye vía `ts.isInstantiatedModule`), same-file import-equals (decidible → ahora cierra) y cross-module (indecidible sin checker → residual) quedan consistentes, no contradictorios.
...
- **B4** — el preload de import-equals no reusaba `producesRuntimeValue` (colectores no-espejados; ver arriba).

**El fix durable NO es cazar los 4 — es centralizar la lógica COMPARTIDA y que cada path la LLAME:** un `valueTransparentChildren` (consultado por el descenso `valueTransparentLeaves` Y el ascenso `isValueTransparentParent`), una `isExemptInDeferredBody` (ramas (c) y (d)), un `NON_ABSENCE_DENIALS` completo, un `producesRuntimeValue` (todos los colectores, incl. import-equals same-file). Tras la centralización, un fix a un path es un fix a todos, y el hunt deja de encontrar la misma raíz disfrazada. **Caveat:** se centraliza lo COMPARTIDO; lo que de verdad difiere (base vs key son posiciones distintas) mantiene su estructura, y la centralización misma pide cuidado (no es gratis). Este es el mecanismo que acaba el whack-a-mole — no más vueltas hasta que un re-hunt sobre el estado centralizado vuelva 0 verificado.
