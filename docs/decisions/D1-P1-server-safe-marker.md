...
- **FP-B shadow-scoping** — const-enum-only namespace que colisiona con un global: `namespaceIsInstantiated` pasaba `preserveConstEnums=false` (decía "elided"), pero el emisor de RUNTIME — **esbuild** (Vite; `tsconfig.build.json` es `emitDeclarationOnly`, tsc NO emite el JS) — **INSTANCIA** el namespace (`var navigator;(IIFE)`; verificado member-only / bare-ref / minify — bajo minify renombra pero declara local) → SOMBREA el global. Fix: pasar `true` para que `ts.isInstantiatedModule` coincida con el emit real de esbuild. Anclado por un test contra `esbuild.transformSync` (el transformer REAL, NO `tsc` — codex P2; el ancla anterior probaba tsc, que es emitDeclarationOnly → falsa confianza): rompe ruidoso si esbuild cambiara su emit o el DS cambiara de transformer. **Soundness:** un namespace SOLO type-only sigue elided → ref bare SIGUE flaggeando.
