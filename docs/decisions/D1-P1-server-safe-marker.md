...
- **Clase B — eval-sink template-selector:** `` g.constructor[`cal${"l"}`](…) `` para `.call`/`.apply`/`.bind` — el gate ya cazaba `["call"]`, `` [`call`] ``, `[(0,"call")]`, `[b?"call":"call"]`; solo la **sustitución de template** escapaba `valueTransparentLeaves` (la key se quedaba sin foldear). Fix: helper `foldConstString` folda `TemplateExpression` con sustituciones constantes (recursivo, + unwrap erased) en `accessedMemberName`. Decidible — NO es el residual data-flow documentado (ese es la indirección de key por variable, no la sustitución de literales). Una key `[k]` real sigue residual.
...
- **CLASE B (2) — eval-sink `+`-concat.** `foldConstString` foldaba string-literal y template pero **no `BinaryExpression` `+`**: `([] as any)["construc"+"tor"]["construc"+"tor"]("return window")` (base `any` → compila) esquivaba el selector del Function-constructor. **Fix:** fold de concat binaria de strings constantes (recursivo, cubre concat dentro de sustitución de template).
