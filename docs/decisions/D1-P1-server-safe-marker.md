...
- **FP-B shadow-scoping** — const-enum-only namespace que colisiona con un global: `namespaceIsInstantiated` pasaba `preserveConstEnums=false`, pero el build real lo fuerza a `true` (`verbatimModuleSyntax` en `tsconfig.build.json` — el config que gobierna el emit de runtime; vite solo bundlea el JS de tsc — → el emit instancia el namespace `var navigator;(IIFE)`, verificado vs emit real) → SOMBREA el global. Fix: pasar `true`, anclado por un test contra el **build-emit real** (rompe ruidoso si el DS migrara a un bundler que inlinea const-enums). **Soundness:** un namespace SOLO type-only sigue elided → ref bare SIGUE flaggeando.
