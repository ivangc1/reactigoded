 ...
 *      puede listar globals añadidos DESPUÉS de Node 22.13.0 (engine
 *      mínimo declarado). Como este test corre en cada celda de la matriz
 *      CI (#151, ubuntu/windows × Node 22.13/24), la celda 22.13 falla si
 *      la whitelist incluye algo que el engine mínimo NO provee — un
 *      consumer en 22.13 recibiría un falso negativo (global tratado como
 *      seguro que en realidad lanza ReferenceError). El anclaje es a 22.13,
...
/**
 * Overclaims de `globals`: nombres que `globals@17.x` lista pero Node 22.13
...
describe("SAFE_GLOBALS whitelist vs Node runtime (#150, fail-closed)", () => {
  it("A. cada SAFE_GLOBAL resuelve en el Node actual (engine-min anchor)", () => {
    ...
    // En la celda Node 22.13 de la matriz CI, cualquier nombre listado por
  ...
  it("F. los GLOBALS_OVERCLAIMS están restados de SAFE_GLOBALS", () => {
    // Invariante directo (no solo el indirecto de Test A en la celda 22.13):
